...
z_min = min(z_world_test) %1029 %TRY 70
z_max = max(z_world_test) %1762 %TRY 20

%When ginette moves deepr in video (Depth/Z_word INCREASES) ... X value decreases in Robot

% Y in Jay
%cross section of image is ZERO
x_min = min(x_world_test) %-410 %TRY -50  
x_max = max(x_world_test) %166  %TRY 50
...
% cross section of image is ZERO
y_min = min(y_world_test) %-165            %TRY 130
y_max = max(y_world_test) %215 %her lowest %TRY 100 

%Step 1 -- move center to 115 
%step 2 -- scale to 100 - 130